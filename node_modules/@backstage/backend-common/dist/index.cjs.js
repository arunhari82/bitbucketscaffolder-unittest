...
const _DefaultCacheClient = class _DefaultCacheClient {
  constructor(client, clientFactory, options) {
    ...
    __privateAdd$1(this, _clientFactory, void 0);
...
class CacheManager {
  constructor(store, connectionString, useRedisSets, logger, errorHandler, defaultTtl) {
     ...
     */
    __publicField$n(this, "storeFactories", {
      redis: this.createRedisStoreFactory(),
      memcache: this.createMemcacheStoreFactory(),
      memory: this.createMemoryStoreFactory()
    });
    __publicField$n(this, "logger");
    __publicField$n(this, "store");
    __publicField$n(this, "connection");
    __publicField$n(this, "useRedisSets");
    __publicField$n(this, "errorHandler");
    __publicField$n(this, "defaultTtl");
...
function isAuthCallbackOptions$1(options) {
  return "onAuth" in options;
}
const _Git$1 = class _Git {
  constructor(config) {
    this.config = config;
    __publicField$m(this, "headers");
    __publicField$m(this, "onAuth");
    __publicField$m(this, "onProgressHandler", () => {
      let currentPhase = "";
      return (event) => {
        var _a, _b;
...
__publicField$m(_Git$1, "fromAuth", (options) => {
  if (isAuthCallbackOptions$1(options)) {
    const { onAuth, logger: logger2 } = options;
    return new _Git$1({ onAuth, logger: logger2 });
  }
  const { username, password, token, logger } = options;
  return new _Git$1({ onAuth: () => ({ username, password }), token, logger });
});
let Git$1 = _Git$1;
...
class MysqlConnector {
  ...
  getRoleConfig(pluginId) {
    var _a;
    return (_a = this.config.getOptionalString(`${pluginPath$3(pluginId)}.role`)) != null ? _a : this.config.getOptionalString("role");
  }
  /**
  ...
  getAdditionalKnexConfig(pluginId) {
    var _a, _b;
    const pluginConfig = (_a = this.config.getOptionalConfig(`${pluginPath$3(pluginId)}.knexConfig`)) == null ? void 0 : _a.get();
  ...
  getEnsureExistsConfig(pluginId) {
    ...
    return (_b = this.config.getOptionalBoolean(`${pluginPath$3(pluginId)}.ensureExists`)) != null ? _b : baseConfig;
  }
...
function createSchemaOverride(client, name) {
  var _a;
  try {
    return (_a = pgConnector.createSchemaOverride) == null ? void 0 : _a.call(pgConnector, name);
  } catch (e) {
    throw new errors.InputError(
...
class PgConnector {
  ...
  getRoleConfig(pluginId) {
    var _a;
    return (_a = this.config.getOptionalString(`${pluginPath$2(pluginId)}.role`)) != null ? _a : this.config.getOptionalString("role");
  }
  /**
   * Provides the knexConfig which should be used for a given plugin.
   *
  ...
  getAdditionalKnexConfig(pluginId) {
    var _a, _b;
    const pluginConfig = (_a = this.config.getOptionalConfig(`${pluginPath$2(pluginId)}.knexConfig`)) == null ? void 0 : _a.get();
    const baseConfig = (_b = this.config.getOptionalConfig("knexConfig")) == null ? void 0 : _b.get();
    return lodash.merge(baseConfig, pluginConfig);
  }
  getEnsureExistsConfig(pluginId) {
    var _a, _b;
    const baseConfig = (_a = this.config.getOptionalBoolean("ensureExists")) != null ? _a : true;
    return (_b = this.config.getOptionalBoolean(`${pluginPath$2(pluginId)}.ensureExists`)) != null ? _b : baseConfig;
  }
  getEnsureSchemaExistsConfig(pluginId) {
    var _a, _b;
    const baseConfig = (_a = this.config.getOptionalBoolean("ensureSchemaExists")) != null ? _a : false;
    return (_b = this.config.getOptionalBoolean(
...
class Sqlite3Connector {
  ...
  getRoleConfig(pluginId) {
    var _a;
    return (_a = this.config.getOptionalString(`${pluginPath$1(pluginId)}.role`)) != null ? _a : this.config.getOptionalString("role");
  }
  /**
   * Provides the knexConfig which should be used for a given plugin.
   *
  ...
  getAdditionalKnexConfig(pluginId) {
    var _a, _b;
    const pluginConfig = (_a = this.config.getOptionalConfig(`${pluginPath$1(pluginId)}.knexConfig`)) == null ? void 0 : _a.get();
    const baseConfig = (_b = this.config.getOptionalConfig("knexConfig")) == null ? void 0 : _b.get();
    return lodash.merge(baseConfig, pluginConfig);
  }
  getPluginDivisionModeConfig() {
...
__publicField$l(_AzureUrlReader, "factory", ({ config, treeResponseFactory }) => {
...
__publicField$k(_BitbucketCloudUrlReader, "factory", ({ config, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
...
__publicField$j(_BitbucketUrlReader, "factory", ({ config, logger, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
...
__publicField$i(_BitbucketServerUrlReader, "factory", ({ config, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
...
const _Git = class _Git {
  constructor(config) {
    this.config = config;
    __publicField$h(this, "headers");
    __publicField$h(this, "onAuth");
    __publicField$h(this, "onProgressHandler", () => {
      let currentPhase = "";
      return (event) => {
        var _a, _b;
        if (currentPhase !== event.phase) {
...
__publicField$h(_Git, "fromAuth", (options) => {
...
__publicField$g(_GerritUrlReader, "factory", ({ config, treeResponseFactory }) => {
  var _a;
  const integrations = integration.ScmIntegrations.fromConfig(config);
...
const _GithubUrlReader = class _GithubUrlReader {
  constructor(integration, deps) {
    this.integration = integration;
    this.deps = deps;
    __publicField$f(this, "getCredentials", async (url, options) => {
...
__publicField$f(_GithubUrlReader, "factory", ({ config, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
...
__publicField$e(_GitlabUrlReader, "factory", ({ config, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
...
__publicField$d(_GiteaUrlReader, "factory", ({ config, treeResponseFactory }) => {
...
const _HarnessUrlReader = class _HarnessUrlReader {
...
__publicField$c(_HarnessUrlReader, "factory", ({ config }) => {
  return integration.ScmIntegrations.fromConfig(config).harness.list().map((integration) => {
    const reader = new _HarnessUrlReader(integration);
    const predicate = (url) => {
      return url.host === integration.config.host;
    };
    return { reader, predicate };
...
let HarnessUrlReader = _HarnessUrlReader;
...
__publicField$b(_AwsS3UrlReader, "factory", ({ config, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
 ...
 */
__publicField$a(_FetchUrlReader, "factory", ({ config }) => {
  var _a, _b;
...
class TarArchiveResponse {
  constructor(stream, subPath, workDir, etag, filter, stripFirstDirectory = true) {
    ...
    __publicField$8(this, "read", false);
    if (subPath) {
      if (!subPath.endsWith("/")) {
        this.subPath += "/";
      }
...
__publicField$5(_GoogleGcsUrlReader, "factory", ({ config, logger }) => {
  ...
  if (!pathMatch) {
    if (!requireGitPath) {
      ...
      return {
        ...
        path: "/",
        commitSpecifier: commitSpecifier2 === "" ? void 0 : commitSpecifier2 == null ? void 0 : commitSpecifier2.replace(/^\/|\/$/g, "")
      };
    }
    throw new Error(
...
const _AwsCodeCommitUrlReader = class _AwsCodeCommitUrlReader {
  constructor(credsManager, integration, deps) {
    this.credsManager = credsManager;
    this.integration = integration;
    this.deps = deps;
  ...
  static async buildCredentials(credsManager, region, integration) {
    ...
    let explicitCredentials;
    if (accessKeyId && secretAccessKey) {
      explicitCredentials = _AwsCodeCommitUrlReader.buildStaticCredentials(
        accessKeyId,
        secretAccessKey
      );
    } else {
  ...
  async buildCodeCommitClient(credsManager, region, integration) {
    const credentials = await _AwsCodeCommitUrlReader.buildCredentials(
      credsManager,
      region,
      integration
    );
  ...
  async readUrl(url, options) {
    ...
    try {
      ...
      if ((options == null ? void 0 : options.etag) && options.etag === response.commitId) {
        throw new errors.NotModifiedError();
      }
      return ReadUrlResponseFactory.fromReadable(
        stream.Readable.from([response == null ? void 0 : response.fileContent]),
        {
  ...
  async readTree(url, options) {
    ...
    try {
      ...
      const allFiles = await this.readTreePath(
        ...
        repositoryName,
        commitSpecifier,
        options == null ? void 0 : options.etag
      );
      const responses = [];
      for (let i = 0; i < allFiles.length; i++) {
        ...
        const objectData = await stream.Readable.from([response == null ? void 0 : response.fileContent]);
        responses.push({
          data: objectData,
          path: posix.relative(
...
__publicField$4(_AwsCodeCommitUrlReader, "factory", ({ config, treeResponseFactory }) => {
  const integrations = integration.ScmIntegrations.fromConfig(config);
  const credsManager = integrationAwsNode.DefaultAwsCredentialsManager.fromConfig(config);
  return integrations.awsCodeCommit.list().map((integration) => {
    const reader = new _AwsCodeCommitUrlReader(credsManager, integration, {
      treeResponseFactory
    });
    const predicate = (url) => {
...
let AwsCodeCommitUrlReader = _AwsCodeCommitUrlReader;
...
class ServerTokenManager {
  ...
  static fromConfig(config, options) {
    var _a, _b;
    const oldSecrets = (_a = config.getOptionalConfigArray("backend.auth.keys")) == null ? void 0 : _a.map((c) => c.getString("secret"));
    const newSecrets = (_b = config.getOptionalConfigArray("backend.auth.externalAccess")) == null ? void 0 : _b.filter((c) => c.getString("type") === "legacy").map((c) => c.getString("options.secret"));
    const secrets = [...oldSecrets != null ? oldSecrets : [], ...newSecrets != null ? newSecrets : []];
    if (secrets.length) {
